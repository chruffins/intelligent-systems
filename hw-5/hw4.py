...
def hw4p1(training_set: list[Image], testing_set: list[Image]):
    ...
    plt.show()
    """
    np.save("classifierl1", p1_network.layers[0].neurons)
    np.save("classifierl2", p1_network.layers[1].neurons)
...
def hw4p2(training_set: list[Image], testing_set: list[Image]):
    ...
    plt.show()
    """
    np.save("autoencoderl1", p2_network.layers[0].neurons)
    np.save("autoencoderl2", p2_network.layers[1].neurons)
